...
void GameLayer::update(float dt)
{
	...
	CCARRAY_FOREACH(this->bulletLayer->m_pAllBullet,bt)
	{
		...
		CCARRAY_FOREACH(this->enemyLayer->m_pAllEnemy2,et)
		{
			...
			if (CCRect::CCRectIntersectsRect(bullet->boundingBox(),enemy2->getBoundingBox()))
			{
				if (enemy2->getLife()>1)
				{
					enemy2->loseLife();
	...
	CCARRAY_FOREACH(this->bulletLayer->m_pAllBullet,bt)
	{
		...
		CCARRAY_FOREACH(this->enemyLayer->m_pAllEnemy3,et)
		{
			...
			if (CCRect::CCRectIntersectsRect(bullet->boundingBox(),enemy3->getBoundingBox()))
			{
				//如果life>1,移除bullet
				if (enemy3->getLife()>1)
				{
	...
	CCARRAY_FOREACH(this->mutiBulletsLayer->m_pAllMutiBullets,mbt)
	{
		...
		{
			Enemy* enemy1=(Enemy*)et;
			if (CCRect::CCRectIntersectsRect(mutiBullets->boundingBox(),enemy1->getBoundingBox()))
			{
				if (enemy1->getLife()==1)
				{
					enemy1->loseLife();
	...
	CCARRAY_FOREACH(this->mutiBulletsLayer->m_pAllMutiBullets,mbt)
	{
		...
		{
			Enemy* enemy2=(Enemy*)et;
			if (CCRect::CCRectIntersectsRect(mutiBullets->boundingBox(),enemy2->getBoundingBox()))
			{
				if (enemy2->getLife()>1)
				{
					enemy2->loseLife();
	...
	CCARRAY_FOREACH(this->mutiBulletsLayer->m_pAllMutiBullets,mbt)
	{
		...
		{
			Enemy* enemy3=(Enemy*)et;
			if (CCRect::CCRectIntersectsRect(mutiBullets->boundingBox(),enemy3->getBoundingBox()))
			{
				//如果life>1,移除bullet
				if (enemy3->getLife()>1)
				{
	...
	CCARRAY_FOREACH(this->enemyLayer->m_pAllEnemy1,et)
	{
		...
		if (enemy1->getLife()>0)
		{
			if (CCRect::CCRectIntersectsRect(airplaneRect,enemy1->getBoundingBox()))
			{
				//卸载所有任务计划
	...
	CCARRAY_FOREACH(this->enemyLayer->m_pAllEnemy2,et)
	{
		...
		if (enemy2->getLife()>0)
		{
			if (CCRect::CCRectIntersectsRect(airplaneRect,enemy2->getBoundingBox()))
			{
				//卸载所有任务计划
				this->unscheduleAllSelectors();
				this->bulletLayer->StopShoot();
	...
	CCARRAY_FOREACH(this->enemyLayer->m_pAllEnemy3,et)
	{
		...
		if (enemy3->getLife()>0)
		{
			if (CCRect::CCRectIntersectsRect(airplaneRect,enemy3->getBoundingBox()))
			{
				//卸载所有任务计划
				this->unscheduleAllSelectors();
				this->bulletLayer->StopShoot();
	...
	CCARRAY_FOREACH(this->ufoLayer->m_pAllMutiBullets,ut)
	{
		...
		if (CCRect::CCRectIntersectsRect(this->planeLayer->getChildByTag(AIRPLANE)->boundingBox(),mutiBullets->boundingBox()))
		{
	...
	CCARRAY_FOREACH(this->ufoLayer->m_pAllBigBoom,ut)
	{
		...
		if (CCRect::CCRectIntersectsRect(this->planeLayer->getChildByTag(AIRPLANE)->boundingBox(),bigBoom->boundingBox()))
		{
